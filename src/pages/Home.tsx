...
import { ArrowRight, ChevronLeft, ChevronRight, ShoppingBag, Zap, Flame, Target, Briefcase, Factory } from "lucide-react";
...
function ProductRail({ title, products, onViewAll, icon }: { title: string; products: any[] | undefined; onViewAll: () => void; icon?: "flame" | "target" }) {
  return (
    <section className="mt-5">
      <div className="flex items-center justify-between mb-2.5 px-4">
        <div className="flex items-center gap-2">
          {icon === "flame" && <Flame className="w-5 h-5 text-red-500 fill-red-500" />}
          {icon === "target" && <Target className="w-5 h-5 text-red-500" />}
          <h2 className="text-lg font-extrabold text-slate-800">{title}</h2>
        </div>
        <button onClick={onViewAll} className="text-peza-orange text-sm font-bold flex items-center gap-0.5 whitespace-nowrap">
          View all <ChevronRight className="w-4 h-4" />
        </button>
      </div>
      <div className="flex gap-2.5 sm:gap-3 overflow-x-auto no-scrollbar px-4 pb-1 snap-x snap-mandatory">
        {(products || []).slice(0, 10).map((p) => (
          <div key={p.id} className="w-[190px] sm:w-[210px] lg:w-[225px] flex-shrink-0 snap-start">
...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-8 bg-slate-50/60">
      {/* Reference order: categories -> slim brand rail -> hero -> merchandising. Header and bottom navigation remain in the existing app shell. */}
      <section className="px-3 sm:px-4 pt-1.5">
        <div className="flex gap-4 sm:gap-5 overflow-x-auto no-scrollbar pb-1 snap-x snap-mandatory">
          {categories?.slice(0, 9).map((cat) => (
            <button key={cat.id} onClick={() => navigate(`/shop?cat=${cat.slug}`)} className="flex flex-col items-center gap-1.5 flex-shrink-0 snap-start min-w-[72px] sm:min-w-[82px]">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center overflow-hidden">
                {cat.image ? <img src={cat.image} alt={cat.name} className="w-10 h-10 sm:w-11 sm:h-11 object-cover rounded-full" /> : <ShoppingBag className="w-6 h-6 text-peza-orange" />}
              </div>
              <span className="text-[11px] sm:text-xs leading-tight text-slate-700 font-medium text-center max-w-[88px]">{cat.name}</span>
            </button>
          ))}
        </div>
      </section>

      <div className="mt-1 border-y border-slate-200 bg-white overflow-hidden h-10 sm:h-11">
          ...
          {[...BRANDS, ...BRANDS].map(([brand, slug, color], i) => (
            <span key={`${brand}-${i}`} className="inline-flex items-center justify-center h-full min-w-[68px] sm:min-w-[78px] px-2">
              <img
                ...
                alt={brand}
                title={brand}
                className="max-h-[18px] sm:max-h-5 max-w-[62px] sm:max-w-[70px] w-auto object-contain"
                loading="lazy"
              />
            </span>
          ))}
      ...
      <section className="relative mx-3 sm:mx-4 mt-3 rounded-xl overflow-hidden h-[210px] sm:h-[300px] lg:h-[340px] cursor-pointer" onClick={goToShop}>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent flex flex-col justify-end p-5 sm:p-7">
          ...
          <h1 className="text-white text-2xl sm:text-4xl font-extrabold mt-1.5">{banners[bannerIdx].title}</h1>
          <p className="text-white/90 text-xs sm:text-base mt-1 max-w-lg">{banners[bannerIdx].subtitle}</p>
          <span className="inline-flex items-center gap-1 bg-white text-peza-brown text-xs font-bold px-4 py-2 rounded-lg mt-3 w-fit shadow-lg">
            {banners[bannerIdx].cta} <ArrowRight className="w-3.5 h-3.5" />
          </span>
        </div>
        <button aria-label="Previous banner" onClick={(e) => { e.stopPropagation(); setBannerIdx((i) => (i - 1 + banners.length) % banners.length); }} className="absolute left-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 text-peza-brown shadow flex items-center justify-center">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <button aria-label="Next banner" onClick={(e) => { e.stopPropagation(); setBannerIdx((i) => (i + 1) % banners.length); }} className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 text-peza-brown shadow flex items-center justify-center">
      ...
      <section className="mt-4 px-3 sm:px-4">
        <div className="flex items-center gap-2 mb-2.5">
          <Zap className="w-5 h-5 text-peza-orange fill-peza-orange" />
          <h2 className="text-lg font-extrabold text-slate-800">Flash Deals</h2>
          <div className="ml-auto flex items-center gap-1">
            {Object.entries(timeLeft).map(([k, v], i) => <span key={k} className="bg-orange-50 text-peza-orange text-xs font-extrabold px-2 py-1 rounded-md border border-orange-100">{String(v).padStart(2, "0")}{i < 2 ? ":" : ""}</span>)}
          </div>
          <button onClick={goToShop} className="text-peza-orange text-sm font-bold flex items-center gap-0.5 ml-1">View all <ChevronRight className="w-4 h-4" /></button>
        </div>
        <div className="flex gap-2.5 sm:gap-3 overflow-x-auto no-scrollbar pb-1 snap-x snap-mandatory">
          {(deals || []).slice(0, 10).map((p) => <div key={p.id} className="w-[190px] sm:w-[210px] lg:w-[225px] flex-shrink-0 snap-start"><PremiumProductCard product={p} /></div>)}
        </div>
      </section>

      <AdBanner index={0} onShop={goToShop} />
      <ProductRail title="Trending Now" products={featured} onViewAll={goToShop} icon="flame" />
      <ProductRail title="Recommended for You" products={electronics?.length ? electronics : featured} onViewAll={goToShop} icon="target" />

      <AdBanner index={1} onShop={() => navigate("/shop?cat=electronics")} />
      <ProductRail title={CATEGORY_ROWS[0].title} products={electronics} onViewAll={() => navigate("/shop?cat=electronics")} />
      <ProductRail title={CATEGORY_ROWS[1].title} products={fashion} onViewAll={() => navigate("/shop?cat=fashion")} />
      <AdBanner index={2} onShop={() => navigate("/shop?cat=beauty")} />
      ...
      <section className="px-3 sm:px-4 mt-6">
        <div className="grid md:grid-cols-2 gap-3">
          <div className="bg-gradient-to-br from-peza-green to-green-700 rounded-xl p-5 text-white cursor-pointer" onClick={() => navigate("/jobs")}>
            ...
            <h3 className="text-lg font-bold">Find Work in Africa</h3>
          ...
          <div className="bg-gradient-to-br from-peza-brown to-peza-brown-light rounded-xl p-5 text-white cursor-pointer" onClick={() => navigate("/suppliers")}>
            ...
            <h3 className="text-lg font-bold">B2B Suppliers Hub</h3>
      ...
      <section className="px-3 sm:px-4 mt-6 mb-8">
        <div className="flex items-center justify-between mb-2.5">
          <h3 className="text-lg font-extrabold text-slate-800">Market Prices</h3>
          <button onClick={() => navigate("/market-prices")} className="text-peza-orange text-sm font-bold flex items-center gap-1">All prices <ChevronRight className="w-4 h-4" /></button>
        </div>
