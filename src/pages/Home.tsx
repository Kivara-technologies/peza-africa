...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      {/* Full-color brand ticker */}
      <div className="mx-4 mt-3 overflow-hidden rounded-2xl border border-peza-cream-dark bg-white shadow-sm">
        <div className="flex animate-ticker whitespace-nowrap py-3">
          {[...BRANDS, ...BRANDS].map(([name, slug, color], i) => (
            <div key={`${slug}-${i}`} className="flex h-12 min-w-[138px] shrink-0 items-center justify-center gap-3 border-r border-peza-cream-dark px-5 sm:min-w-[160px]">
              <img
                src={`https://cdn.simpleicons.org/${slug}/${color}`}
                alt={`${name} logo`}
                className="h-7 w-auto max-w-[78px] object-contain sm:h-8 sm:max-w-[92px]"
                loading="lazy"
              />
              <span className="text-[11px] font-semibold tracking-wide text-peza-brown">{name}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {categories?.slice(0, 8).map((cat) => (
            <button key={cat.id} onClick={() => navigate(`/shop?cat=${cat.slug}`)} className="flex flex-col items-center gap-2 flex-shrink-0 group">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-peza-gold-light to-peza-gold flex items-center justify-center border-2 border-transparent group-hover:border-peza-orange transition-all shadow-md">
                {cat.image ? <img src={cat.image} alt={cat.name} className="w-10 h-10 object-cover rounded-full" /> : <span className="text-xl">🛍️</span>}
              </div>
      ...
      {/* Flash Deals */}
      <section className="px-4 mt-6">
        <div className="flex items-center gap-3 mb-3"><Zap className="w-5 h-5 text-peza-orange" /><h3 className="text-lg font-bold text-peza-brown">Flash Deals</h3><div className="ml-auto flex gap-1">{Object.entries(timeLeft).map(([k, v], i) => <span key={k} className="bg-peza-orange text-white text-xs font-bold px-2 py-1 rounded">{String(v).padStart(2, "0")}{i < 2 ? ":" : ""}</span>)}</div></div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{deals?.slice(0, 4).map((p) => <PremiumProductCard key={p.id} product={p} />)}</div>
      </section>

      {/* Trending Now */}
      <section className="px-4 mt-6">
        <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-peza-orange" /><h3 className="text-lg font-bold text-peza-brown">Trending Now</h3></div><button onClick={goToShop} className="text-peza-orange text-sm font-semibold flex items-center gap-1">View all <ChevronRight className="w-4 h-4" /></button></div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{featured?.slice(0, 8).map((p) => <PremiumProductCard key={p.id} product={p} />)}</div>
      </section>
