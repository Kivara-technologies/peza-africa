...
import { Search, SlidersHorizontal, ShieldCheck, Truck, Star, Sparkles } from "lucide-react";
...
export default function Shop() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pb-8">
      <div className="rounded-2xl bg-gradient-to-r from-peza-brown via-peza-brown-light to-peza-brown p-4 text-white shadow-lg mb-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] text-peza-gold font-bold">Marketplace</p>
            <h1 className="text-2xl font-extrabold mt-1">Shop the best deals</h1>
          </div>
          <div className="rounded-full bg-white/10 border border-white/15 px-3 py-1.5 text-xs font-semibold text-peza-gold">
            {products?.length || 0} live items
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
        {[
          ...
          { icon: Truck, label: "Fast Zambian delivery" },
          { icon: Star, label: "Top-rated picks" },
        ].map(({ icon: Icon, label }) => (
          <div key={label} className="flex items-center gap-3 rounded-2xl border border-peza-cream-dark bg-white px-4 py-3 shadow-sm">
            <div className="w-9 h-9 rounded-full bg-peza-orange/10 flex items-center justify-center text-peza-orange">
              <Icon className="w-4 h-4" />
            </div>
            <span className="text-sm font-semibold text-peza-brown">{label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 flex items-center gap-2 bg-white border border-peza-cream-dark rounded-xl px-4 py-2.5 shadow-sm">
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl border flex items-center gap-2 transition-colors shadow-sm ${showFilters ? "bg-peza-orange text-white border-peza-orange" : "bg-white text-peza-brown border-peza-cream-dark"}`}
        >
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-bold text-peza-brown">Browse by category</span>
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-peza-orange">
