...
function requireRider(role: string) {
  if (role !== "rider") throw new TRPCError({ code: "FORBIDDEN", message: "Rider account required" });
}
...
export const riderRouter = router({
  ...
  updateLocation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [row] = await ctx.db.insert(schema.riderLocations)
        .values({ riderId: ctx.user.id, lat: input.lat.toString(), lng: input.lng.toString(), isOnline: true })
        .onConflictDoUpdate({ target: schema.riderLocations.riderId, set: { lat: input.lat.toString(), lng: input.lng.toString(), isOnline: true, updatedAt: new Date() } })
        .returning();
      return row;
    }),
  ...
  goOffline: protectedProcedure.mutation(async ({ ctx }) => {
    requireRider(ctx.user.role);
    await ctx.db.update(schema.riderLocations).set({ isOnline: false }).where(eq(schema.riderLocations.riderId, ctx.user.id));
    return { success: true };
  }),
  ...
  availableOrders: protectedProcedure.query(async ({ ctx }) => {
    ...
    return ctx.db.select({
      ...
      .orderBy(desc(schema.orders.createdAt)).limit(50);
  }),
  ...
  claimOrder: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (location && order.deliveryLat !== null && order.deliveryLng !== null) {
        const km = distanceKm(Number(location.lat), Number(location.lng), Number(order.deliveryLat), Number(order.deliveryLng));
        earning = Math.max(earning, RIDER_BASE_ZMW + RIDER_PER_KM_ZMW * km);
      }
      earning = Number(earning.toFixed(2));

      const [claimed] = await ctx.db.update(schema.orders)
        .set({ riderId: ctx.user.id, riderEarning: earning.toFixed(2), status: "shipped" })
        .where(and(eq(schema.orders.id, input.orderId), isNull(schema.orders.riderId), inArray(schema.orders.status, [...CLAIMABLE_STATUSES])))
        .returning();
  ...
  myDeliveries: protectedProcedure.query(async ({ ctx }) => {
    requireRider(ctx.user.role);
    return ctx.db.select().from(schema.orders)
  ...
  history: protectedProcedure.query(async ({ ctx }) => {
    ...
    return ctx.db.select({
      ...
      orderNumber: schema.orders.orderNumber,
      total: schema.orders.total,
      riderEarning: schema.orders.riderEarning,
      deliveryAddress: schema.orders.deliveryAddress,
      status: schema.orders.status,
      createdAt: schema.orders.createdAt,
    }).from(schema.orders)
      .where(and(eq(schema.orders.riderId, ctx.user.id), eq(schema.orders.status, "delivered")))
  ...
  earnings: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [result] = await ctx.db.select({
      total: sql<string>`coalesce(sum(${schema.orders.riderEarning}), 0)`,
      trips: sql<number>`count(*)`,
    }).from(schema.orders).where(and(eq(schema.orders.riderId, ctx.user.id), eq(schema.orders.status, "delivered")));
    return { total: Number(result?.total ?? 0), trips: Number(result?.trips ?? 0), currency: "ZMW" as const };
  }),
  ...
  markDelivered: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [updated] = await ctx.db.update(schema.orders).set({ status: "delivered" }).where(eq(schema.orders.id, input.orderId)).returning();
