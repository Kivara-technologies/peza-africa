...
const applicationInput = z.object({
  ...
  vehicleRegistration: z.string().trim().max(30).optional(),
...
export const riderRouter = router({
  apply: protectedProcedure.input(applicationInput).mutation(async ({ ctx, input }) => {
    ...
    let application;
    if (existing[0]) {
      [application] = await ctx.db.update(schema.riderApplications).set({ ...input, status: "pending", reviewedBy: null, reviewedAt: null, reviewNote: null, updatedAt: now }).where(eq(schema.riderApplications.userId, ctx.user.id)).returning();
    } else {
