...
// byte-by-byte, which is a real attack surface on a webhook signature
// check. Length is compared first (timingSafeEqual throws on mismatched
...
paymentWebhookRoutes.post("/mobile-money", async (c) => {
  ...
  // Idempotency: if this provider reference has already been recorded on
  // another transaction, this is a replay (or a duplicate webhook delivery,
  // which providers do send) — do not process it twice. Requires the
  // unique index added in 0009_webhook_idempotency.sql.
  const [alreadyProcessed] = await db
    .select({ id: schema.walletTransactions.id })
    .from(schema.walletTransactions)
    .where(eq(schema.walletTransactions.providerReference, body.reference))
    .limit(1);
  if (alreadyProcessed) {
...
paymentWebhookRoutes.post("/infobip/sms", async (c) => {
  ...
  if (!secret) {
    return c.json({ error: "Infobip webhook is not configured yet" }, 503);
  }
...
paymentWebhookRoutes.get("/infobip/test", async (c) => {
  // Debug/dev-only endpoint — it echoes catalog content with no auth, which
  // is fine locally but not something to leave open in production.
