...
export const vendorRouter = router({
  ...
  updateProduct: protectedProcedure.input(productInput.partial().extend({ id: z.number() })).mutation(async ({ ctx, input }) => {
    ...
    if (rest.categorySlug) {
      const [category] = await ctx.db.select().from(schema.categories).where(eq(schema.categories.slug, rest.categorySlug)).limit(1);
      if (!category) throw new TRPCError({ code: "BAD_REQUEST", message: "Unknown category" });
  ...
  salesSummary: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [summary] = await ctx.db.select({ totalRevenue: sql<string>`coalesce(sum(${schema.orderItems.total}), 0)`, unitsSold: sql<string>`coalesce(sum(${schema.orderItems.quantity}), 0)`, ordersCount: sql<string>`count(distinct ${schema.orderItems.orderId})` }).from(schema.orderItems).where(inArray(schema.orderItems.productId, productIds));
  ...
  topProducts: protectedProcedure.query(async ({ ctx }) => {
    const rows = await ctx.db.select({ productId: schema.orderItems.productId, productName: schema.orderItems.productName, unitsSold: sql<string>`coalesce(sum(${schema.orderItems.quantity}),0)`, revenue: sql<string>`coalesce(sum(${schema.orderItems.total}),0)` }).from(schema.orderItems).innerJoin(schema.products, eq(schema.products.id, schema.orderItems.productId)).where(eq(schema.products.vendorId, ctx.user.id)).groupBy(schema.orderItems.productId, schema.orderItems.productName).orderBy(desc(sql`sum(${schema.orderItems.total})`)).limit(5);
